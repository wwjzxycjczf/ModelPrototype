...
/**
 * MathSessionPane is a visual component that provides a scrolling In/Out
 * session interface to the Mathematica kernel. Much more sophisticated than the
 * kernel's own "terminal" interface, it provides features such as full text
 * editing of input including copy/paste and unlimited undo/redo, support for
 * graphics, control of fonts and styles, customizable syntax coloring, and
 * bracket matching.
 * <p>
 * To use a MathSessionPane, either supply an existing KernelLink using the
 * setLink() method, or provide a command line to create a link via the
 * setLinkArguments() or setLinkArgumentsArray() methods. You must always
 * arrange for the connect() method to be called, which will launch the kernel
 * (if you supplied an appropriate command line) and cause the first In[] prompt
 * to appear.
 * <p>
 ...
 * 			// If we're quitting while the kernel is busy, it might not die when
 * 			// the link
 ...
 * 	msp.setLinkArgumentsArray(new String[] { &quot;-linkmode&quot;, &quot;launch&quot;,
 * 			&quot;-linkname&quot;, &quot;\&quot;d:/math41/mathkernel\&quot; -mathlink&quot; });
 * 	try {
 * 		msp.connect();
 ...
 * </PRE>
 * 
 * To see a more complete application built around a MathSessionPane, look at
 * the SimpleFrontEnd example application in
 * JLink/Examples/Part2/SimpleFrontEnd.
 * 
 * @since 2.0
 */
...
//	private MathSessionTextPane.MTDocument doc;
	...
	/**
	 * Creates a new MathSessionPane with the specified policies for vertical
	 * and horizontal scrollbars.
	 * 
	 * @param vsbPolicy
	 *            vertical scrollbar policy (see the Swing JScrollPane class)
	 * @param hsbPolicy
	...
	public MathSessionPane(int vsbPolicy, int hsbPolicy) {
		...
		// The only PropertyChangeEvent that anyone here fires is for starting
		// and finishing an eval.
		// We register with the textPane so that clients can register with us,
		// and this class appears to be
		// the source of the events. In this way, we let the textPane itself
		// fire the events for clients who
		// want to use it directly, not via this ScrollPane wrapper, but we
		// re-fire them for the majority of
		// users who are using this class.
		textPane.addComputationPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				propChangeSupport.firePropertyChange(evt.getPropertyName(),
						evt.getOldValue(), evt.getNewValue());
			}
		});

		// We need our own ScrollPaneLayout to accommodate the timing panel in
		...
		ScrollPaneLayout layout = new ScrollPaneLayout() {
			public void layoutContainer(Container parent) {
				...
				if (sb != null) {
					...
					sb.setBounds(r.x + timingPanelSize, r.y, r.width
							- timingPanelSize, r.height);
					lowerLeft.setBounds(0, r.y, timingPanelSize, r.height);
					// Mac OS X has a lot of repaint problems right now. I don't
					// even know why this works, but
					// it seems to dramatically improve appearance.
					if (Utils.isMacOSX() && isShowTiming())
						RepaintManager.currentManager(MathSessionPane.this)
								.markCompletelyClean(MathSessionPane.this);
				}
		...
		final JPanel timingPanel = new JPanel() {
			public void paintComponent(Graphics g) {
				String s = Double.toString(textPane.getLastTiming())
						+ " seconds";
		...
		// This is the property listener that causes display of computation
		// timings.
		textPane.addComputationPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
		...
		// We need to redo the syntax coloring if the window is resized. For
		// performance reasons we only
		// color the text that is actually visible. Therefore, when new text is
		// made visible we need to re-color.
	...
	/**
	 * Sets the link to use for communicating with Mathematica. Use this method
	 * if you already have a link you want to use, or you want to open the link
	 * manually. Most programmers will use setLinkArguments() or
	 * setLinkArgumentsArray instead and let this class open the link itself.
	 * Even if you open the link yourself and call setLink(), you must still
	 ...
	 * Null if no link is open yet.
	 */
	public KernelLink getLink() {
		return textPane.getLink();
	}

	// Why are there two forms for the setLinkArguments method and not just one
	// sig that takes Object? Because
	// I want to present an API that is bean-friendly. Bean property editors
	// will display the String form
	// in an editable way. In other words, if there was only one property
	// "linkArguments" that took an Object, it would not
	// be usable in bean property editors.

	/**
	 * Sets the command line to use to open a link to Mathematica. The string is
	 * in the same form as you would pass to MathLinkFactory.createKernelLink(),
	 * for example:
	 * 
	 * <pre>
	 ...
	 * 
	 * @param linkArgs
	 *            the command line to create the link
	 * @see #setLinkArgumentsArray(String[])
	...
	/**
	 * Sets the arguments to use to open a link to Mathematica. The string array
	 * is in the same form as you would pass to
	 * MathLinkFactory.createKernelLink(), for example:
	 * 
	 * <pre>
	...
	/**
	 * Sets the number of milliseconds that the connect() method will wait to
	 * connect the link to Mathematica. If the timeout expires, connect() will
	 * throw a MathLinkException. You can use this timeout to prevent your
	 * program from hanging indefinitely in certain circumstances if the kernel
	 * does not launch properly. The default value is very large, effectively no
	 * timeout.
	 * 
	...
	/**
	 * Returns the number of milliseconds that the connect() method will wait to
	 * connect the link to Mathematica.
	 * 
	 ...
	 * The JTextPane is the actual component that handles all text input and
	 * output. Advanced programmers might want access to the JTextPane to call
	 ...
	 * might want to know this for several reasons. First, they might want to
	 * disable/enable parts of their user interface, and second, they might want
	 * to know whether the Mathematica kernel is currently busy. This latter
	 * consideration is particularly important if other parts of the program
	 * want to use the same link. Programmers who are considering using this
	 * method might find it preferable instead to use a PropertyChangeListener
	 * to monitor the changes in the computationActive property.
	 * 
	...
	/**
	 * Sets the color for input and output text. This color is used for input
	 * except when syntax coloring is on and the characters are in a syntax
	 * class that is specially colored. The default is black.
	 * 
	 * @param c
	 */
	...
	/**
	 * Returns the color that will be used for literal strings in input if
	...
	/**
	 * Sets the color that will be used for literal strings in input if syntax
	 * coloring is on. The default is a shade of blue.
	 * 
	 * @param c
	 */
	...
	/**
	 * Returns the color that will be used for comments in input if syntax
	 * coloring is on.
	 */
	...
	/**
	 * Sets the color that will be used for comments in input if syntax coloring
	 * is on. The default is a shade of green.
	 * 
	 * @param c
	 */
	...
	/**
	 * Returns the color that will be used in input for symbols in the
	 * Mathematica System` context if syntax coloring is on.
	 */
	...
	/**
	 * Sets the color that will be used in input for symbols in the Mathematica
	 * System` context if syntax coloring is on. The default is a shade of
	 * purple.
	 * 
	 * @param c
	 */
	...
	/**
	 * Returns the amount that input and output will be indented from the left
	 * edge.
	 * 
	...
	/**
	 * Sets the amount that input and output will be indented from the left
	 * edge. The default is 20.
	 * 
	 * @param indent
	...
	/**
	 * Sets whether to scale graphics so as to fit into the current visible
	...
	/**
	 * Indicates whether graphics are set to be scaled to fit into the current
	 * visible width.
	 */
	...
	/**
	 * Sets whether to use the Mathematica front end in the background to assist
	 * in rendering graphics. This must be set to true if you want typeset
	 * output (such as plot labels) in your graphics. It will also improve the
	 * quality of most images, particularly surface graphics. Setting this to
	 * true will cause the front end to launch. The default is false.
	 * 
	 * @param b
	 */
	...
	/**
	 * Indicates whether the Mathematica front end will be used to assist in
	 * rendering graphics.
	 */
	...
	/**
	 * Lets you customize syntax coloring by specifying an array of symbol names
	 * and the color you want them drawn in when they appear in input. As an
	 * example, you could use this to specify that all the symbols from a
	 * certain context should be rendered in red. You can call this method as
	 * many times as you like, specifying many different combinations of symbols
	 * and colors.
	 * 
	 * @param syms
	 *            The symbols to be specially colored, for example {"If", "For",
	 *            "Do"}
	 * @param c
	...
	/**
	 * Wipes out all syntax coloring customizations specified by
	 * addColoredSymbols().
	 * 
	...
	/**
	 * Specifies whether to display the wall-clock timing for each input in a
	 * timing panel at the lower left corner of the pane. The default is true.
	 * 
	 * @param show
	 */
	...
	/**
	 * Indicates whether evaluation timings will be shown in a timing panel at
	 * the lower left corner of the pane.
	 */
	...
	/**
	 * Evaluates the current input. This operation is automatically mapped to
	...
	/**
	 * Redoes the last undone edit. This operation is automatically mapped to
	 * the Ctrl-Y key combination (Cmd-Y on Macintosh).
	 */
	public void redo() {
		textPane.redo();
	}
	...
	/**
	 * Copies the previous input to the current cursor location. This operation
	 * is automatically mapped to the Ctrl-L key combination (Cmd-L on
	 * Macintosh).
	 */
	...
	/**
	 * Grows the current selection to highlight the nearest matching set of
	 * delimiters. This is done in a way that respects Mathematica syntax,
	 * meaning that delimiters within comments and strings are ignored. This
	 * operation is automatically mapped to the Ctrl-B key combination (Cmd-B on
	 * Macintosh).
	 */
	...
	/**
	 * Adds a PropertyChangeListener that will be notified of changes in the
	 * computationActive property. You can use this to receive notifications
	 * when computations start and end. For eample, you might want to enable or
	 * disable parts of your user interface, or lock out other parts of your
	 * program from trying to use the link.
	 * 
	 * @param listener
	 */
	...
	/**
	 * Connects the link to Mathematica and prepares the pane for use. The first
	 * In[] prompt will not appear and the pane will not be editable until this
	 * method has been called, even if you have supplied a preexisting link
	 * using the setLink() method. If you used setLinkArguments() or
	 * setLinkArgumentsArray() to specify link parameters, then the link will be
	 * created and connected at this time. The Mathematica kernel will be
	...
	/**
	 * Closes the link to Mathematica. The link is not automatically closed
	 * anywhere in this class, so you must arrange for this method to be called.
	 */
...
class MathSessionTextPane extends JTextPane {
	...
	private PropertyChangeSupport computationPropChangeSupport = new PropertyChangeSupport(
			this);
	...
	private boolean isInputMode; // Whether we are in state of user editing
									// input
	...
	MathSessionTextPane() {
		...
		keymap.addActionForKeyStroke(
				KeyStroke.getKeyStroke(KeyEvent.VK_Z, cmdKey), undoAction);
		...
		keymap.addActionForKeyStroke(
				KeyStroke.getKeyStroke(KeyEvent.VK_Y, cmdKey), redoAction);
		...
		keymap.addActionForKeyStroke(
				KeyStroke.getKeyStroke(KeyEvent.VK_B, cmdKey), bracketAction);
		...
		keymap.addActionForKeyStroke(
				KeyStroke.getKeyStroke(KeyEvent.VK_L, cmdKey), cifaAction);
		...
		keymap.addActionForKeyStroke(
				KeyStroke.getKeyStroke(KeyEvent.VK_PERIOD, abortKey),
				abortAction);
		...
		keymap.addActionForKeyStroke(
				KeyStroke.getKeyStroke(KeyEvent.VK_COMMA, abortKey),
				interruptAction);
		setKeymap(keymap);
	...
	void setBackgroundColor(Color c) {
		...
		setBackground(c);
		// XOR cursor.
		setCaretColor(new Color(backgroundColor.getRed() ^ 255,
	...
	private void setInputMode(boolean isInputMode) {
		...
		if (oldValue != isInputMode)
			computationPropChangeSupport.firePropertyChange(
					"computationActive", new Boolean(!oldValue), new Boolean(
							!isInputMode));
	}
	...
	public void addComputationPropertyChangeListener(
			PropertyChangeListener listener) {
		computationPropChangeSupport.addPropertyChangeListener(listener);
	}

	public void removeComputationPropertyChangeListener(
			PropertyChangeListener listener) {
	...
	void connect() throws MathLinkException {
		...
		try {
			...
			// Rethrow the excpetion so the caller knows there was a problem,
			// but restore the cursor first.
		...
		ml.evaluate("$Line");
		// This loop handles the cases where an InputNamePacket arrives at the
		// start or not.
		...
		// This PacketListener does the work of writing the output to the
		// screen.
		ml.addPacketListener(new PktHandler(this, getDoc()));
		// Install the interrupt dialog (you get this from Alt-comma, like in
		// the front end).
		Frame parentFrame = (Frame) SwingUtilities.getAncestorOfClass(
				Frame.class, this);
		Dialog parentDialog = (Dialog) SwingUtilities.getAncestorOfClass(
				Dialog.class, this);
		if (parentFrame != null)
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				// Determine and cache the width of a char and height of a line
				// in the font being used. This is a weird way
				// to do this, but I cannot find another way to accurately
				// determine char widths and heights. AWT-based methods
				// like Font.getMaxCharBounds() don't give sizes that match the
				// fonts used in the JTextPane. There must be a
				...
				try {
					...
					lineHeight = (int) (modelToView(getDoc().getLength())
							.getY() - modelToView(0).getY());
				} catch (BadLocationException e) {
				}
	...
	// Next two are overrides that give this pane the ability to turn off word
	// wrap.
	// Got this from some code by Jacob Smullyan for the SkunkDAV project
	// (sourceforge).
	...
	Dimension getVisibleTextBounds() {
		...
		Point endVisible = new Point(startVisible.x + v.getSize().width,
				startVisible.y + v.getSize().height);
	...
	void evaluateInput() {
		...
		// Even though pane is not editable when not in input mode, we are doing
		// an end-run around
		...
		if (getSelectionStart() >= firstEditPos
				&& getSelectionEnd() >= firstEditPos) {
			// Need to put in the carriage return at end of input, plus another
			// one for spacing.
			try {
			...
			// Would like to call setEditable(false) here, but that causes a
			// beep on Mac OSX.
	...
	private void abortEval() {
		...
		if (!isInputMode()) {
			try {
				...
				e.printStackTrace();
				// Don't call clearError(), as that is synchronized, and the
				// link is locked by another thread.
			}
		}
	}
	...
	void balanceBrackets() {
		...
		if (getSelectionStart() >= inputStart)
			result = bracketMatcher.balance(getSelectionStart() - inputStart,
		...
		if (isInputMode()) {
			...
			if (getSelectionStart() >= firstEditPos
				...
				if (inputStyleStart != 0 && inputStyleEnd != 0) {
					try {
						String s = getText(inputStyleStart, inputStyleEnd
								- inputStyleStart);
						// Should always end with two carriage returns inserted
						// during evaluateInput(). Remove them.
						if (s.endsWith("\n\n"))
	...
	// EvalTask is the class whose Run method performs the computation and
	// printing of output.
		...
		private void preEval() throws MathLinkException {
			...
			String df = "(LinkWrite[$ParentLink, DisplayPacket[EvaluateToImage[#, "
					...
					+ (fitGraphics ? ", ImageSize->{"
							+ (getParent().getSize().width - getLeftIndent() - 10)
							+ ", Automatic}"
							: "") + "]]]; #)&";
	...
	// Need a custom document class to prevent edits when the selection touches
	// the non-editable
	// portion of the document (anything up to, and including, the last In[]
	// prompt).

	class MTDocument extends DefaultStyledDocument {

		private int firstEditPos; // First position at which editing is legal
									// (right after most recent In[] prompt).
		...
		// This call lets clients know whether doc changes currently in progress
		// are due to internal
		// machinations of the document rather than user events. They can use
		// this information to determine
		// whether to record undoable events that may be generated. If this
		// returns false, then they probably
		...
		public void setEvalInput(String s){
			EvalInput = s;
		}
		String getEvalInput() {
			try {
...
//				EvalInput;
		...
		// syntax runs (that is, iterating
		// tokenizer.hasMoreRecords()) is very quick (easily tens of thousands
		// of characters can be scanned
		// every keystroke). What is costly is setting character attributes.
		// Therefore some steps are taken
		// to minimize the amount of text that is actively styled. We do some
		// work to only style text that is
		// currently visible. This means that we must ensure that this method is
		...
		void doSyntaxColor() {
			...
			// If syntax coloring is off, just set all text to normal attribute
			// and return. We only even get here if
			// syntax coloring was previoulsy on and has now been turned off.
			if (!isSyntaxColoring()) {
				recordUndos = false;
				try {
					setCharacterAttributes(firstEditPos, getLength()
							- firstEditPos, attrNormal, false);
				} finally {
					recordUndos = false;
			...
			// All indices are offsets from firstEditPos. These two variables
			// are the starting and ending indices
			...
			// This can happen if we are called from a scroll event and the view
			// has not been updated.
			// firstEditPos has been set, but the viewport has not been moved to
			// show the insertion point yet.
			...
			// These tests allow a simple optimization in the case of a scroll:
			// only style text in the newly-revealed area
			// at the top or bottom of the last styled range. None of these
			// branches will be taken if an edit has
			// occurred--the whole visible area will be re-styled.
			if (colorsHaveChanged) {
				// If colors have changed, skip all the machinations withe
				// lastStyledStart and visible region. Will
				// need to redo whole input.
				startChar = 0;
				endChar = getLength() - firstEditPos;
			} else if (firstVisibleChar >= lastStyledStart
					&& lastVisibleChar <= lastStyledEnd) {
				return;
			} else if (firstVisibleChar < lastStyledStart) {
				endChar = lastVisibleChar > lastStyledEnd ? lastVisibleChar
						: lastStyledStart;
			} else if (lastVisibleChar > lastStyledEnd) {
				startChar = firstVisibleChar < lastStyledStart ? firstVisibleChar
						: lastStyledEnd;
			}
...
//			String input = "CForm[ToExpression[Import[\"d:\\1.mml\",\"MathML\"]]]";
			tokenizer.setText(input);

			recordUndos = false;
			try {
				setCharacterAttributes(firstEditPos + startChar, endChar
						- startChar, attrNormal, false);

				AttributeSet curAttr = attrNormal;
				AttributeSet attr = null;
				while (tokenizer.hasMoreRecords()) {
					SyntaxTokenizer.SyntaxRecord rec = tokenizer
							.getNextRecord();
					switch (rec.type) {
					...
					case SyntaxTokenizer.SYMBOL: {
						attr = attrNormal;
						String sym = input.substring(rec.start, rec.start
								+ rec.length);
					...
					if (attr != attrNormal
							&& attr != curAttr
							&& intervalsIntersect(rec.start, rec.start
									+ rec.length, startChar, endChar))
						setCharacterAttributes(rec.start + firstEditPos,
								rec.length, attr, false);
					curAttr = attr;
				}
			} finally {
				recordUndos = true;
			...
			lastStyledStart = Math.min(lastStyledStart,
					Math.min(startChar, firstVisibleChar));
			lastStyledEnd = Math.max(lastStyledEnd,
					Math.max(endChar, lastVisibleChar));
		}

	} // End of MTDocument.

	private static boolean intervalsIntersect(int a_start, int a_end,
...
class PktHandler implements PacketListener {
	...
	public boolean packetArrived(PacketArrivedEvent evt) {
		// All work involving the link must be done here, not in handleString or
		// handleImage.
		// Those execute on the event-dispatch thread, and we will get deadlock
		// over contention for ml.
		...
		try {
			...
			case MathLink.DISPLAYPKT: {
				// DISPLAYPKT is hijacked to supply the result of
				// EvaluateToImage.
	...
	private void handleString(final int pkt, final String s,
			final boolean lastWasMessage) throws InvocationTargetException,
			InterruptedException {

		// In typical circumstances, this code is called from the EvalTask
		// thread, not the
		// event dispatch thread, so we must use invokeAndWait(). In some cases,
		// however,
		// this code can be called on the dispatch thread, so we must test for
		// this and if
		// true we just invoke directly.
		if (SwingUtilities.isEventDispatchThread()) {
			// To get this called on the event dispatch thread, use a
			// MathSessionPane in an
			// InstallJava situation. Because we add the packet listener that
			// calls this code to
			// the link itself (the UI link), it will be called for packets
	...
	private void handleString0(final int pkt, final String s,
			final boolean lastWasMessage) {
		...
		try {
			...
			case MathLink.INPUTNAMEPKT:
				...
				if (doc.getLength() >= 2
						&& !doc.getText(doc.getLength() - 2, 2).equals("\n\n"))
			...
			case MathLink.MESSAGEPKT:
				...
				// Don't insert the string--the actual text comes in the
				// subsequent TextPacket.
				break;
			}
		} catch (BadLocationException e) {
			e.printStackTrace();
	...
	private void handleImage(final byte[] imageData)
	...
	private void handleImage0(final byte[] imageData) {
		...
		try {
			...
			// Next 5 lines are mostly borrowed from the implementation of
			// insertIcon() in JTextPane.
			// I can't use that method because it works only on the selection
			// (doesn't allow
